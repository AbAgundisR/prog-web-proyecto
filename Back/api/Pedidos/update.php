...
if (
    !empty($data->pedido_ID) && !empty($data->subtotal) && !empty($data->iva)
...
$sql = "UPDATE Pedidos SET subtotal = ?, iva = ?,
    total = ?, direccion = ?, ciudad = ?,
    estado = ?, cp = ?, telefono = ?, WHERE pedido_ID = ?";
...
if ($stmt = mysqli_prepare($db, $sql)) {
    ...
    $param_pedido_ID = $pedido->pedido_ID;

    // Attempt to execute the prepared statement
    if (mysqli_stmt_execute($stmt)) {
        // tell the user
        echo json_encode(array("message" => "Pedido actualizada."));
    } else {
